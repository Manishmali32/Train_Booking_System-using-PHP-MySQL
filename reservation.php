<?php
    ...
    if(isset($_REQUEST['submit'])) { 
        ...
        if (!$result) {
            echo "<div class='alert alert-danger'>No rooms available for the selected dates.</div>";
        }
    }
?>
...
<head>
    ...
    <script>
      $(function() {
        ...
        // Set check-out date to be after check-in
        $("input[name='checkin']").datepicker({
          dateFormat: 'yy-mm-dd',
          minDate: 0,
          onSelect: function(selectedDate) {
            $("input[name='checkout']").datepicker("option", "minDate", selectedDate);
          }
        });
...
<body>
    <div class="container">
        ...
        <?php   
            if(isset($_REQUEST['submit']) && $result) {
                if(mysqli_num_rows($result) > 0) {
                    while($row = mysqli_fetch_array($result)) {
                        $room_cat = $row['room_cat'];
                        $stmt = $user->db->prepare("SELECT * FROM room_category WHERE roomname = ?");
                    ...
                    echo "<div class='alert alert-warning'>No rooms available for the selected dates.</div>";
                }
            }
        ?>
    </div>
